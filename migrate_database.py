...
class DatabaseMigrator:
    ...
    def backfill_usernames(self):
        """Backfill usernames from email if username is missing"""
        with self.engine.begin() as conn:
            ...
            """))
            logger.info("Backfilled usernames from email")
    
    def backfill_points(self):
        """Backfill points from completed sessions"""
        with self.engine.begin() as conn:
            conn.execute(text("""UPDATE users
            SET points = cs.session_count * 10
            FROM (
            ...
            """))
        logger.info("Backfilled points from completed sessions")
    ...
    def run_migration(self):
        ...
        try:
            ...
            # Step 5: Data backfill if needed
            logger.info("Step 5: Backfilling data...")
