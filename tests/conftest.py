...
# Custom type to handle UUIDs in SQLite (store as string)
class UUIDAdapter(TypeDecorator):
    impl = String
    cache_ok = True
...
# Replace ARRAY and JSONB types with JSON for SQLite
...
# Override array, JSONB, and UUID implementation in models for testing
for table in Base.metadata.tables.values():
    for column in table.columns:
        if isinstance(column.type, ARRAY):
            column.type = ArrayAdapter()
        elif isinstance(column.type, JSONB):
            column.type = ArrayAdapter()  # Use ArrayAdapter for JSONB too
        elif isinstance(column.type, PG_UUID):
            column.type = UUIDAdapter()
